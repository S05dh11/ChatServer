...
public:
    ...
    // 通过会话ID获取会话元数据（不加载历史消息）
    std::shared_ptr<Session>  getSessionMeta(const std::string& sessionId);
    // 往某个会话中添加消息
    ...
    // 获取指定会话的消息数量
    size_t getMessageCount(const std::string& sessionId)const;
    // 获取指定会话的第一条用户消息
    std::string getFirstUserMessage(const std::string& sessionId)const;
