...
bool QwenProvider::initModel(const std::map<std::string, std::string>& modelConfig){
    ...
    _isAvailable = true;
    INFO("QwenProvider initModel success, endpoint: {}", _endpoint);
    return true;
}
...
std::string QwenProvider::getModelDesc() const{
    return "阿里云通义千问大模型，能力均衡全面，适合通用问答、文本生成与知识推理";
}

// 发送消息 - 全量返回
std::string QwenProvider::sendMessage(const std::vector<Message>& messages, const std::map<std::string, std::string>& requestParam)
{
    // 1. 检测模型是否可用
    if(!isAvailable()){
    ...
    INFO("QwenProvider sendMessage requestBody: {}", requestBodyStr);
    ...
    // 6. 发送POST请求
    auto response = client.Post("/compatible-mode/v1/chat/completions", headers, requestBodyStr, "application/json");
    if(!response){
        ERR("QwenProvider sendMessage POST request failed");
        return "";
    }
    INFO("QwenProvider sendMessage POST request success, status : {}", response->status);
    INFO("QwenProvider sendMessage POST request success, body : {}", response->body);
    ...
    if(Json::parseFromStream(readerBuilder, responseStream, &responseBody, &parseError)){
        if(responseBody.isMember("choices") && responseBody["choices"].isArray() && !responseBody["choices"].empty()){
            ...
            if(choice.isMember("message") && choice["message"].isMember("content")){
                ...
                INFO("QwenProvider response text: {}", replyContent);
                return replyContent;
            }
        }
    }
    ...
    // 8. json解析失败
    ERR("QwenProvider sendMessage POST response body parse failed");
    return "";
}

// 发送消息 - 增量返回 - 流式响应
std::string QwenProvider::sendMessageStream(const std::vector<Message>& messages,
                                             ...
                                             std::function<void(const std::string&, bool)> callback)
{
    ...
    writerBuilder["indentation"] = "";
    std::string requestBodyStr = Json::writeString(writerBuilder, requestBody);
    INFO("QwenProvider sendMessageStream requestBody: {}", requestBodyStr);
    ...
    req.content_receiver = [&](const char* data, size_t len, size_t offset, size_t totalLength){
        ...
        INFO("QwenProvider sendMessageStream buffer: {}", buffer);
        ...
        while((pos = buffer.find("\n\n")) != std::string::npos){
            ...
            if(chunk.compare(0, 6, "data: ") == 0){
                ...
                if(Json::parseFromStream(reader, modelDataStream, &modelDataJson, &errors)){
                    ...
                    WARN("QwenProvider sendMessageStream parse modelDataJson error: {}", errors);
                }
            }
        }
        return true;
