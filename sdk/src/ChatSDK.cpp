...
void ChatSDK::registerAllProvider(const std::vector<std::shared_ptr<Config>>& configs){
    ...
    // doubao-seed-2-0-pro
    if(!_llmManager.isModelAvailable("doubao-pro")){
        auto doubaoProvider = std::make_unique<DoubaoProvider>();

        // unuqe_ptr不允许自动类型转换，即使类型之间存在继承关系
        _llmManager.registerProvider("doubao-pro", std::move(doubaoProvider));
        INFO("doubao-seed-2-0-pro provider registered successed");
    }

    // qwen3.6-plus
    if(!_llmManager.isModelAvailable("qwen3.6-plus")){
        auto qwenProvider = std::make_unique<QwenProvider>();

        // unuqe_ptr不允许自动类型转换，即使类型之间存在继承关系
        _llmManager.registerProvider("qwen3.6-plus", std::move(qwenProvider));
        INFO("qwen3.6-plus provider registered successed");
    }
    ...
    for(const auto& config : configs){
        if(auto apiConfig = std::dynamic_pointer_cast<APIConfig>(config)){
            if(apiConfig->_modelName == "deepseek-chat" ||
               apiConfig->_modelName == "doubao-pro" ||
               apiConfig->_modelName == "qwen3.6-plus"){
                 // 支持的云端模型
