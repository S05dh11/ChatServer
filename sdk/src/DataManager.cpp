    ...
    , _db(nullptr)
{
    ...
    // 启用SQLite外键约束，使ON DELETE CASCADE生效
...
// 获取指定会话的第一条用户消息
std::string DataManager::getFirstUserMessage(const std::string& sessionId)const{
    std::lock_guard<std::mutex> lock(_mutex);
...
// 获取指定会话的消息数量
size_t DataManager::getSessionMessageCount(const std::string& sessionId)const{
    std::lock_guard<std::mutex> lock(_mutex);
