...
ChatServer::ChatServer(const ServerConfig& config){
    ...
    qwenConfig->_apiKey = config.qwenAPIKey;
    qwenConfig->_temperature = config.temperature;
    qwenConfig->_maxTokens = config.maxTokens;
...
{
    // 获取会话列表 - 只获取元数据，不加载历史消息
    std::vector<std::string> sessionIDs = _chatSDK->getSessionLists();

    Json::Value dataArray(Json::arrayValue);
    for(const auto& sessionID : sessionIDs){
        auto session = _chatSDK->_sessionManager.getSessionMeta(sessionID);
        if(session){
            ...
            sessionJson["message_count"] = static_cast<int>(_chatSDK->_sessionManager.getMessageCount(sessionID));
            sessionJson["first_user_message"] = _chatSDK->_sessionManager.getFirstUserMessage(sessionID);
