...
bool validateConfig(ai_chat_server::ServerConfig& config) {
    ...
    // 验证至少有一个API密钥不为空
    if (config.deepseekAPIKey.empty() && config.doubaoAPIKey.empty() && config.qwenAPIKey.empty()) {
...
int main(int argc, char** argv) {
    try {
        ...
        // 先初始化日志组件，确保validateConfig中可以使用ERR
        ...
        INFO("  Doubao API Key: {}", (config.doubaoAPIKey.empty() ? "未设置" : "已设置"));
        INFO("  Qwen API Key: {}", (config.qwenAPIKey.empty() ? "未设置" : "已设置"));
