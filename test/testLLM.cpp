...
TEST(DoubaoProviderTest, sendMessage){
    auto provider = std::make_shared<ai_chat_sdk::DoubaoProvider>();
    ASSERT_TRUE(provider != nullptr);

    std::map<std::string, std::string> modelParam;
    modelParam["api_key"] = std::getenv("doubao_apikey");
    modelParam["endpoint"] = "https://ark.cn-beijing.volces.com/api/v3";
...
TEST(QwenProviderTest, sendMessage){
    auto provider = std::make_shared<ai_chat_sdk::QwenProvider>();
    ASSERT_TRUE(provider != nullptr);

    std::map<std::string, std::string> modelParam;
    modelParam["api_key"] = std::getenv("qwen_apikey");
    modelParam["endpoint"] = "https://dashscope.aliyuncs.com/compatible-mode/v1";
...
TEST(ChatSDKTest, sendMessage){
    ...
    // 配置支持的模型参数：云模型-deepseek-chat doubao-seed-2-0-pro qwen3.6-plus   Ollama本地接入deepseek-r1:1.5b
    // deepseek-chat
    ...
    doubaoConfig->_modelName = "doubao-pro";
    ...
    qwenConfig->_modelName = "qwen3.6-plus";
